...
typedef DWORD       HSHADER; //shader effect
typedef DWORD       HSHTECH; //shader technique
typedef DWORD       HSHPARAM; //shader parameter
...
#define HGESHADER_SAVE 0
#define HGESHADER_DONTSAVE 1
#define HGESHADER_DONTSAVESAMPLER 2
#define HGESHADER_DONTSAVESHADER 3
...
enum hgeBoolState
{
	...
	HGE_WINDOWCAPTION   = 30,   // bool     add window caption? (default: true)
	HGE_ACCEPTFILES     = 31,   // bool     accept drag. files? (default: false)
...
{
    hgeBumpVertex v[4];
    HTEXTURE        tex, bumpmap;
    void * hTangents[4], * hBiNormals[4], * hNormals[4];
};
...
public:
	...
	virtual void*		HGE_CALL	Resource_Load(const char *filename, DWORD *size=0) = 0;
	...
	virtual float       HGE_CALL    Timer_GetDeltaRealtime() = 0;
	virtual int			HGE_CALL	Timer_GetFPS() = 0;

	virtual HEFFECT		HGE_CALL	Effect_Load(const char *filename, DWORD size=0) = 0;
	...
	virtual HMUSIC		HGE_CALL	Music_Load(const char *filename, DWORD size=0) = 0;
	...
	virtual HSTREAM		HGE_CALL	Stream_Load(const char *filename, DWORD size=0) = 0;
	...
	virtual HSURFACE    HGE_CALL    Target_GetSurface(HTARGET target) = 0;

	virtual void        HGE_CALL    Surface_Free(HSURFACE surf) = 0;
	virtual DWORD*      HGE_CALL    Surface_Lock(HSURFACE surf, bool bReadOnly=true, int left=0, int top=0, int width=0, int height=0) = 0;
	virtual void        HGE_CALL    Surface_Unlock(HSURFACE surf) = 0;

	virtual HTEXTURE	HGE_CALL	Texture_Create(int width, int height) = 0;
	virtual HTEXTURE	HGE_CALL	Texture_Load(const char *filename, DWORD size=0, bool bMipmap=false) = 0;
	...
	virtual DWORD*		HGE_CALL	Texture_Lock(HTEXTURE tex, bool bReadOnly=true, int left=0, int top=0, int width=0, int height=0) = 0;
