...
HGE* HGE_CALL hgeCreate(const int ver) {
    if (ver == HGE_VERSION) {
        return static_cast<HGE*>(HGE_Impl::interface_get());
    }
    return nullptr;
}
...
void HGE_CALL HGE_Impl::System_SetStateInt(const hgeIntState state, const int value) {
    ...
    case HGE_FPS:
        ...
        hgefps_ = value;
        if (hgefps_ > 0) {
            fixed_delta_ = int(1000.0f / value);
        }
        else {
            fixed_delta_ = 0;
        }
...
void HGE_Impl::post_error(char* error) {
    System_Log(error);
    strcpy(error_, error);
}
...
LRESULT CALLBACK WindowProc(const HWND hwnd, const UINT msg, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_PAINT:
        if (pHGE->d3d_ && pHGE->proc_render_func_ && pHGE->windowed_) {
            pHGE->proc_render_func_();
        }
        break;
    ...
    case WM_ACTIVATE: {
        ...
        if (pHGE->d3d_ && pHGE->active_ != activating) {
            pHGE->focus_change(activating);
        }
        return FALSE;
    }
    ...
    case WM_KEYDOWN:
        ...
        return FALSE;
    case WM_SYSKEYUP:
        pHGE->build_event(INPUT_KEYUP, wparam, HIWORD(lparam) & 0xFF, 0, -1, -1);
        return FALSE;
    case WM_KEYUP:
        pHGE->build_event(INPUT_KEYUP, wparam, HIWORD(lparam) & 0xFF, 0, -1, -1);
        return FALSE;
    ...
    case WM_LBUTTONDOWN:
        SetFocus(hwnd);
        pHGE->build_event(INPUT_MBUTTONDOWN, HGEK_LBUTTON, 0, 0, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;
    case WM_MBUTTONDOWN:
        SetFocus(hwnd);
        pHGE->build_event(INPUT_MBUTTONDOWN, HGEK_MBUTTON, 0, 0, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;
    case WM_RBUTTONDOWN:
        SetFocus(hwnd);
        pHGE->build_event(INPUT_MBUTTONDOWN, HGEK_RBUTTON, 0, 0, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;

    case WM_LBUTTONDBLCLK:
        pHGE->build_event(INPUT_MBUTTONDOWN, HGEK_LBUTTON, 0, HGEINP_REPEAT, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;
    case WM_MBUTTONDBLCLK:
        pHGE->build_event(INPUT_MBUTTONDOWN, HGEK_MBUTTON, 0, HGEINP_REPEAT, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;
    case WM_RBUTTONDBLCLK:
        pHGE->build_event(INPUT_MBUTTONDOWN, HGEK_RBUTTON, 0, HGEINP_REPEAT, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;

    case WM_LBUTTONUP:
        pHGE->build_event(INPUT_MBUTTONUP, HGEK_LBUTTON, 0, 0, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;
    case WM_MBUTTONUP:
        pHGE->build_event(INPUT_MBUTTONUP, HGEK_MBUTTON, 0, 0, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;
    case WM_RBUTTONUP:
        pHGE->build_event(INPUT_MBUTTONUP, HGEK_RBUTTON, 0, 0, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;

    case WM_MOUSEMOVE:
        pHGE->build_event(INPUT_MOUSEMOVE, 0, 0, 0, LOWORDINT(lparam), HIWORDINT(lparam));
        return FALSE;
    case 0x020A: // WM_MOUSEWHEEL, GET_WHEEL_DELTA_WPARAM(wparam);
        pHGE->build_event(INPUT_MOUSEWHEEL, short(HIWORD(wparam)) / 120, 0, 0, LOWORDINT(lparam),
                          HIWORDINT(lparam));
        return FALSE;
